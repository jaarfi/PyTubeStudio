...
class MoveModelRequestData(_ModelRequests):
    ...
    """
    size: float = Field(default=-80, ge=-100, le=100)
    """The size the model should shrink/enlargen to, -70 is a fairly normal size (provided its absolute)"""
...
class PinInfo(_ModelRequests):
    model_id: Optional[str] = Field(default=None, serialization_alias="modelID")
    art_mesh_id: Optional[str] = Field(default=None, erialization_alias="artMeshID")
    angle: float = Field(default=0, ge=0, le=360)
    size: float = Field(default=0, ge=-1, le=1)
